          ...
          {COLLEGES_DATA.map((college, idx) => {
            ...
            return (
              <div
                key={college.id}
                id={college.id}
                className="bg-white rounded-3xl border border-slate-200/80 overflow-hidden shadow-subtle hover:shadow-card-hover transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-0"
              >
                {/* College Image Column */}
                <div
                  className={`lg:col-span-5 relative min-h-[300px] bg-slate-100 ${
                    idx % 2 === 1 ? 'lg:order-2' : ''
                  }`}
                >
                  <img
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-white/90 backdrop-blur-md rounded-full text-xs font-extrabold text-[#123B6D] shadow">
                ...
                <div className="lg:col-span-7 p-8 sm:p-10 flex flex-col justify-between space-y-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {collegePrograms.map((p) => (
                          <button
                            ...
                            className="px-3 py-1 bg-slate-100 hover:bg-[#123B6D] hover:text-white text-slate-700 text-xs font-semibold rounded-lg transition-colors cursor-pointer"
                          >
                            {p.name} ({p.category})
                          </button>
                        ))}
                  ...
                  <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#123B6D] hover:bg-[#1E4D8C] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow transition-all flex items-center space-x-1.5 cursor-pointer"
                    >
